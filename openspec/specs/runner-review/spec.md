# runner-review Specification

## Purpose
How a runner-built change is exposed for review: a local `review/<change-name>` branch that tracks the commit in the latest 🔀 merge request, a sensitive-paths warning in that message, and a diff-only review rule. Review branches are never pushed.
## Requirements
