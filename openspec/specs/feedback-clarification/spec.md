# feedback-clarification Specification

## Purpose
Let the planner ask a feedback submitter exactly one clarifying question, approved by the admin in Telegram, and capture the answer (button or free text) so the submission can be planned with the user's actual intent.
## Requirements
